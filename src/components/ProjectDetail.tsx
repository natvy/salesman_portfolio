...
export default function ProjectDetail({
  ...
  useEffect(() => {
    const unsubscribe = rawScrollY.on("change", (value) => {
      ...
      const extendedIndex = Math.round(
        (value + center - ITEM_HEIGHT / 2) / ITEM_HEIGHT
      );
  ...
  const handleVerticalScroll = () => {
    ...
    scrollTimeoutRef.current = window.setTimeout(() => {
      const center = getCenterY();
      const extendedIndex = Math.round(
        (scrollTop + center - ITEM_HEIGHT / 2) / ITEM_HEIGHT
      );
    ...
    <div className="min-h-screen pt-32">
      <div className="max-w-6xl mx-auto px-8">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div
            ref={verticalRef}
            className="lg:w-2/3 h-[75vh] overflow-y-scroll relative scroll-smooth"
          >
